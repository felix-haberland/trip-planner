...
- **Window reorder rewrites `slots.window_index` (F014)**: `yearly.crud.reorder_windows` reshuffles the `year_plans.windows` JSON array AND remaps every slot's `window_index` across all options of that plan inside one transaction. Don't ever update the array in isolation — the integer indices on every Slot in every Option will silently point at the wrong window. Same caveat for any future windows refactor: a Slot's `window_index` is a position into the parent YearPlan's array, not a stable identifier.
- **TripPlan.position keeps server_default=0 on purpose (F014)**: drag-and-drop reorder writes the column, but the bundled `trips.seed.db` seed loader does a raw `SELECT * → INSERT` and would otherwise NOT NULL on fresh deploys. `crud.create_trip` assigns `max(position) + 1` for new rows; bulk imports land at 0 and tie-break on `id` in `list_trips`. Don't try to remove the server_default "for cleanliness" — fresh deploys will break.
